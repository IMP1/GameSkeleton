...
public class WindowResizable extends WindowMovable {
	...
	public WindowResizable(int x, int y, int width, int height) {
		this(x, y, width, height, MIN_WIDTH, MIN_HEIGHT, width, height);
	}
	
	public WindowResizable(int x, int y, int width, int height, int minWidth, int minHeight, int maxWidth, int maxHeight) {
	...
	private boolean hoverResize(int mx, int my) {
		...
		left = (mx >= x - HOVER_LEEWAY && mx <= x + HOVER_LEEWAY);
		right = (mx >= x + width - HOVER_LEEWAY && mx <= x + width + HOVER_LEEWAY);
		top = (my >= y - HOVER_LEEWAY && my <= y + HOVER_LEEWAY);
		bottom = (my >= y + height - HOVER_LEEWAY && my <= y + height + HOVER_LEEWAY);
	...
	private void handleResizing(int mx, int my) {
		...
		if (mx >= x - HOVER_LEEWAY && mx <= x + HOVER_LEEWAY) left = true;
		if (mx >= x + width - HOVER_LEEWAY && mx <= x + width + HOVER_LEEWAY) right = true;
		if (my >= y - HOVER_LEEWAY && my <= y + HOVER_LEEWAY) top = true;
		if (my >= y + height - HOVER_LEEWAY && my <= y + height + HOVER_LEEWAY) bottom = true;
